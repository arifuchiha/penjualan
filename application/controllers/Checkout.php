		...
		if ($this->input->post('submit', TRUE) == 'Submit')
      {
         ...
         if ($this->form_validation->run() == TRUE)
         {
            ...
            if ($get->num_rows() > 0)
            {
               ...
               $config['useragent'] = 'CV TMoptical';
               $config['protocol'] = 'smtp';
               $config['mailtype'] = 'html';
               $config['smtp_host'] = 'ssl://smtp.gmail.com';
               $config['smtp_port'] = '465';
               $config['smtp_timeout'] = '5';
               $config['smtp_user'] = 'arifbsi061@gmail.com'; //isi dengan email gmail
               $config['smtp_pass'] = 'uchihaitachi'; //isi dengan password
               $config['crlf'] = "\r\n";
               $config['newline'] = "\r\n";
               ...
               $this->email->from('arifbsi061@gmail.com', "CV TMoptical");
               $this->email->to($user->email);
               $this->email->subject('Pembayaran');
               $this->email->message(
                  'Terima Kasih telah melakukan pemesanan di toko kami, selanjutnya silahkan anda melakukan pembayaran melalui transfer uang senilai <b>Rp. '.number_format($total, 0, ',', '.').',-</b> ke no. rekening <b>90xxxx</b> paling lambat '.$bts.' agar pesanan anda bisa kami proses. Detail pembayaran sebagai berikut :<br/><br/>
   ...
   public function city()
   {
      ...
      curl_setopt_array($curl, array(
        ...
        CURLOPT_HTTPHEADER => array(
          "key: 0c82f084dbdf323b2b537cd10410363d"
        ),
      ));
